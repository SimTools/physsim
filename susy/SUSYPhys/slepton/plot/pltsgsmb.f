...
C--
      ...
      DO 100 IRS = 0, NRS
         RS = RSMN + IRS*DRS
         CALL SGSMSM(RS,POLE,AM(1),CF,QF,T3F,SG0,R)
         CALL SGSMSB(MODE,RS,POLE,AM(1),CF,QF,T3F,SG)
...
C*
      ...
      SUBROUTINE SGSMSB(MODE,RS,POLE,AM,CF,QF,T3F,SGEFF)
 
      IMPLICIT     REAL*8 ( A-H, O-Z )
      INTEGER*4    MODE
      REAL   *8    RS, POLE, AM(2), CF, QF, T3F, SGEFF
C--
      REAL   *4    XRS
C--
      PARAMETER    ( NRSH = 200 )
...
C--
         DO 100 IRGN = 1, 3
            ...
            DO 10 IRSH = 0, MRSH
               RSH = RSHMN + DRSH*IRSH
               CALL SGSMSM(RSH,POLE,AM,CF,QF,T3F,SG0,R)
               SGDAT(IRSH,IRGN) = SG0
10          CONTINUE
100      CONTINUE
      ENDIF
