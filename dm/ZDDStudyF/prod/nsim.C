...
{ 
  ...
  stringstream sfname;
  sfname << "zddsim" << setw(3) << setfill('0') << irun << ".root" << ends;
  ...
  JSFSteer jsf;
  JSFLCFULL full;
  ZDDSpring spring("ZDDSpring");

  stringstream slname;
  if (irun == 1) {
    slname << "zddsim.root" << ends;
  } else {
    slname << "zddsim" << setw(3) << setfill('0') << irun-1 << ".root" << ends;
  }
  ...
  printf(" Roots is %g\n",((ZDDBases*)spring.GetBases())->GetEcmInit());
