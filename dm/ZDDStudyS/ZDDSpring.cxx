...
Bool_t ZDDSpringBuf::SetPartons()
{
...
#else
  Int_t    idh     = 220000;                      // PDG code for H
#endif
  ...
  new (partons[1]) JSFSpringParton(2, idh, mass,    0., *qp[1], 0, 0,  0,    0,   0,     0);
           ...
           fR_ISR_side(0)
{
  ...
  ins.clear();
  ins.str(gJSF->Env()->GetValue("ZDDBases.Cs","0.105")); 	 // Cs
  ins >> fCs;
...
Complex_t ZDDBases::FullAmplitude()
{
   ...
   Complex_t amp = AmpEEtoZDD(em, ep, h1, h2, zf);
                               ...
                               const HELVector  &zf)
{
   ...
   HELScalar hh(h1, h2, gddh, fMassH, 0.);
