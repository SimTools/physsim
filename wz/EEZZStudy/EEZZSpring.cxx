...
EEZZSpring::EEZZSpring(const char *name, const char *title,
			 EEZZBases *bases)
  : JSFSpring(name, title, bases)
{
  fEventBuf = new EEZZSpringBuf("EEZZSpringBuf", 
	 "EEZZSpring event buffer", this);
  if( !bases ) { 
    EEZZBases *bs=new EEZZBases();
    SetBases(bs);
  }
}
...
Bool_t EEZZSpring::Initialize()
{
  ...
  if (fFile->IsWritable()) {
    EEZZBases *bs = (EEZZBases *)GetBases();
...
//_____________________________________________________________________________
void EEZZSpringBuf::Spevnt(Int_t &iret) { spevnt_(&iret); }
...
// Get parameters from jsf.conf, if specified.
  ...
  sscanf(gJSF->Env()->GetValue("EEZZBases.ISRBM","3"),"%d",&fISRBM);
  sscanf(gJSF->Env()->GetValue("EEZZBases.ACC1","0.2"),"%lg",&fACC1);
  sscanf(gJSF->Env()->GetValue("EEZZBases.ACC2","0.1"),"%lg",&fACC2);
  sscanf(gJSF->Env()->GetValue("EEZZBases.ITMX1","5"),"%d",&fITMX1);
  sscanf(gJSF->Env()->GetValue("EEZZBases.ITMX2","5"),"%d",&fITMX2);
  sscanf(gJSF->Env()->GetValue("EEZZBases.NCALL","80000"),"%d",&fNCALL);
  ...
  if ( fISRBM == 1 ) {
  	...
  	printf("EEZZBases: Invalid ISRBM = %d\n",fISRBM);
  	printf("    Will STOP immediately\n");
  	exit(1);
  }

//
// Get EEZZ specific parameters.
//
  Char_t pname[40];
  for(Int_t i=0;i<fNDIM;i++){
    sprintf(pname,"EEZZBases.X%i2.2Range",i+1);
    sscanf(gJSF->Env()->GetValue(pname,"0.0 1.0"),"%lg%lg",&fXL[i],&fXU[i]);
  }

  sscanf(gJSF->Env()->GetValue("EEZZBases.Roots","500."),"%lg",&fRoots);
  sscanf(gJSF->Env()->GetValue("EEZZBases.PolElectron","0."),"%lg",&fPolElectron);
  sscanf(gJSF->Env()->GetValue("EEZZBases.SigmaEbeam","0.005"),"%lg",&fSigmaEbeam);
  sscanf(gJSF->Env()->GetValue("EEZZBases.Z1ModesLo","1"),"%d",&fZ1ModesLo);
  sscanf(gJSF->Env()->GetValue("EEZZBases.Z1ModesHi","12"),"%d",&fZ1ModesHi);
  sscanf(gJSF->Env()->GetValue("EEZZBases.Z2ModesLo","1"),"%d",&fZ2ModesLo);
  sscanf(gJSF->Env()->GetValue("EEZZBases.Z2ModesHi","12"),"%d",&fZ2ModesHi);
  sscanf(gJSF->Env()->GetValue("EEZZBases.Alphai","128."),"%lg",&fAlphai);
  sscanf(gJSF->Env()->GetValue("EEZZBases.Alphas","0.120"),"%lg",&fAlphas);
  sscanf(gJSF->Env()->GetValue("EEZZBases.MassW","80.0"),"%lg",&fMassW);
  sscanf(gJSF->Env()->GetValue("EEZZBases.MassZ","91.18"),"%lg",&fMassZ);
  sscanf(gJSF->Env()->GetValue("EEZZBases.MassHiggs","9999."),"%lg",&fMassHiggs);
  sscanf(gJSF->Env()->GetValue("EEZZBases.MassTop","170."),"%lg",&fMassTop);  
}


//_____________________________________________________________________________
void EEZZBases::PrintParameters()
{
//  Print parameters
//
  
  printf("Parameters for ee->EEZZ generator\n");
  printf("  Roots  = %g (GeV)\n",fRoots);
...
//_____________________________________________________________________________
Double_t EEZZBases::Func()		// new style not yet implemented
{
  cerr << ":::::: ERROR "
       << "  EEZZBases::Func() not implemented !!!"
...
//_____________________________________________________________________________
Double_t EEZZBases::Func(Double_t x[])
{
//  Bases Integrand
//
  double val=func_(x);
