...
//  C-tor
// --------------------------
NNZSpring::NNZSpring(const char     *name,
                     const char     *title,
                           NNZBases *bases)
           : JSFSpring(name, title, bases)
{
  fEventBuf = new NNZSpringBuf("NNZSpringBuf",
                               "NNZSpring event buffer",
                               this);
  if (!bases) { 
    SetBases(new NNZBases);
  }
}
...
Bool_t NNZSpring::Initialize()
{
  ...
  if (fFile->IsWritable()) {
    NNZBases *bs = static_cast<NNZBases *>(GetBases());
...
//  SetPartons
// --------------------------
Bool_t NNZSpringBuf::SetPartons()
{
  TClonesArray &partons = *fPartons;
  NNZBases     *bases   = static_cast<NNZBases *>(
                          static_cast<NNZSpring *>(Module())->GetBases());
  ...
  new (partons[2]) JSFSpringParton(3,  idz, rq2z,    0., *qp[4], 2, 4,  0,    0,    0,      0);
  new (partons[3]) JSFSpringParton(4, idf3,   m3, chrg3, *qp[2], 0, 0,  3, hel3, icfz, islevz);
  new (partons[4]) JSFSpringParton(5, idf4,   m4, chrg4, *qp[3], 0, 0,  3, hel4, icfz, islevz);
           ...
           fR_ISR_side(0)
{
  ...
  stringstream ins(gJSF->Env()->GetValue("NNZBases.Ecm","500.")); // E_cm (1TeV)
  ins >> fEcmInit;

  ins.str("");
  ins.clear();
  ins.str(gJSF->Env()->GetValue("NNZBases.Beamstrahlung","1")); // BmStr (on)
  ins >> fBeamStr;

  ins.str("");
  ins.clear();
  ins.str(gJSF->Env()->GetValue("NNZBases.BeamWidth","0.002")); // BmStr (on)
  ins >> fBeamWidth;

  ins.str("");
  ins.clear();
  ins.str(gJSF->Env()->GetValue("NNZBases.Bremsstrahlung","1"));// ISR (on)
  ins >> fISR;

  ins.str("");
  ins.clear();
  ins.str(gJSF->Env()->GetValue("NNZBases.Pole","0."));         // electron polarization
  ins >> fPole;

  ins.str("");
  ins.clear();
  ins.str(gJSF->Env()->GetValue("NNZBases.ZModesLo","1"));      // Z decay mode lo
  ins >> fZModesLo;

  ins.str("");
  ins.clear();
  ins.str(gJSF->Env()->GetValue("NNZBases.ZModesHi","12"));     // Z decay mode hi
  ins >> fZModesHi;

  ins.str("");
  ins.clear();
  ins.str(gJSF->Env()->GetValue("NNZBases.ACC1","0.05"));
  ins >> fACC1;

  ins.str("");
  ins.clear();
  ins.str(gJSF->Env()->GetValue("NNZBases.ACC2","0.05"));
  ins >> fACC2;

  ins.str("");
  ins.clear();
  ins.str(gJSF->Env()->GetValue("NNZBases.ITMX1","20"));
  ins >> fITMX1;

  ins.str("");
  ins.clear();
  ins.str(gJSF->Env()->GetValue("NNZBases.ITMX2","40"));
  ins >> fITMX2;

  ins.str("");
  ins.clear();
  ins.str(gJSF->Env()->GetValue("NNZBases.NCALL","80000"));
  ins >> fNCALL;
...
Double_t NNZBases::Func()
{
  ...
  // --------------------------------------------
  Double_t qz    = TMath::Sqrt(fQ2Z);
  Double_t xilo  = TMath::Log(qz*(qz+m1+m2)/s);
  ...
  Double_t sigma = DSigmaDX(zbranch);
  ...
  Xh_fill( 8, TMath::Sqrt(fQ2Z) , (bsWeight*sigma));
  Xh_fill( 9, fCosZF            , (bsWeight*sigma));
  Xh_fill(10, fPhiZF            , (bsWeight*sigma));
...
//  DSigmaDX
// --------------------------
Double_t NNZBases::DSigmaDX(GENBranch &zbranch)
{
  // --------------------------------------------
  //  Phase space
  // --------------------------------------------
  ...
  Double_t q2z  = zbranch.GetQ2();  // q^2_{Z}
  Double_t qz   = TMath::Sqrt(q2z);
  ...
  Double_t x1, x2;
  if (fEta1 > -fEta2) {
    Double_t rximn = (qz-m1+m2)*(qz+m1+m2)/s;
    Double_t rximx = 1. - 2*m1/rs;
    x1    = 1. - rxi;
    x2    = (rxi - q2z/s)/(1-x1*(1.-cs12)/2.);
    if (1.-x2 < rximn || 1.-x2 > rximx) return 0.;
    weight *= s*x1*x2*x2/(rxi - q2z/s);
  } else {
    Double_t rximn = (qz+m1-m2)*(qz+m1+m2)/s;
    Double_t rximx = 1. - 2*m2/rs;
    x2    = 1. - rxi;
    x1    = (rxi - q2z/s)/(1-x2*(1.-cs12)/2.);
    if (1.-x1 < rximn || 1.-x1 > rximx) return 0.;
    weight *= s*x2*x1*x1/(rxi - q2z/s);
  }
...
//  FullAmplitude()
// --------------------------
Complex_t NNZBases::FullAmplitude()
{
   ...
   Complex_t amp = AmpEEtoNNZ(em, ep, ne, neb, zf);
...
//  AmpEEtoNNZ()
// --------------------------
Complex_t NNZBases::AmpEEtoNNZ(const HELFermion &em,
                               const HELFermion &ep,
                               const HELFermion &ne,
...
void NNZBases::Userin()
{
  ...
  if (fBeamStr) {
    stringstream ins(gJSF->Env()->GetValue("NNZBases.BeamstrahlungFilepath",
                                  "/proj/soft/jsf/pro/data/bsdata/"));
    ins >> bsfiledir;

    ins.clear();
    ins.str(gJSF->Env()->GetValue("NNZBases.BeamstrahlungFilename","trc500"));
    ins >> bsfilename;
  ...
  Xh_init( 8,    70.,   110.,       50, "mz"    );
  Xh_init( 9,    -1.,    +1.,       50, "coszf" );
  Xh_init(10,     0.,   k2Pi,       50, "phizf" );
  Xh_init(11,     0.,     2.,        2, "Helin ");
  Xh_init(12,     0.,     4.,        4, "Helot ");
...
//  SelectHelicities
// --------------------------
void NNZBases::SelectHelicities(Double_t &weight)
{
