...
ClassImp(ZHH6JAnalysis)

ZHH6JAnalysis::ZHH6JAnalysis(const Char_t *name, const Char_t *title)
...
//_____________________________________________________________________
ZHH6JAnalysis::~ZHH6JAnalysis()
{
}

//_____________________________________________________________________
Bool_t ZHH6JAnalysis::Initialize()
{
  //--
...
Bool_t ZHH6JAnalysis::Process(Int_t ev)
{
  //--
  ...
  if (!hEvt) {
    ...
    tupstr << "nev:ecm:ntracks:evis:pt:pl:elmax:ycut:chi2"              << ":"
	   ...
	   << "pj5e:pj5x:pj5y:pj5z:pj6e:pj6x:pj6y:pj6z"                 << ends;
  ...
  while ((h1p = static_cast<ANLPair *>(h1candidates()))) {
    ...
    while ((h2p = static_cast<ANLPair *>(h2candidates()))) {
      ...
      h2.LockChildren();
      ANLPairCombiner zcandidates(h2candidates);
      zcandidates.Reset();
      while ((zp = static_cast<ANLPair *>(zcandidates()))) {
        ...
        if (!((j1p->GetTag() == -4 && j2p->GetTag() == -4) ||
              (j1p->GetTag() == -4 && j2p->GetTag() == -5) ||
              (j1p->GetTag() == -5 && j2p->GetTag() == -4) ||
        ...
        solutions.Add(new ANLPair(new ANLPair(h1p,h2p),new ANLPair(z),chi2));
      }
      h2.UnlockChildren();
    }
    h1.UnlockChildren();
  ...
  hStat->Fill(++selid);
  if (gNgoods == 0) {
    gCutName[(Int_t)selid] << "|m_jj - m_H| <= " << fM2jCut
  ...
  while ((solp = static_cast<ANLPair *>(nextsol()))) {
    ANLPair &sol = *solp;
    ANLPair &hh  = *static_cast<ANLPair *>(sol[0]);
    ANLPair &h1  = *static_cast<ANLPair *>(hh [0]);
    ANLPair &h2  = *static_cast<ANLPair *>(hh [1]);
...
#if 1
  cerr << "(h1j1,h1j2; h2j1,h2j2,zj1,zj2) = ("
       << h1j1p->GetTag() << ","
       << h1j2p->GetTag() << ";"
       << h2j1p->GetTag() << ","
       << h2j2p->GetTag() << ";"
...
//_________________________________________________________
Bool_t ZHH6JAnalysis::Terminate()
{
