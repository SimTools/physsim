...
void UserInitialize()
{
  ...
  gROOTS=gJSF->Env()->GetValue("NNZHBases.Roots",0.0);
  gStoreBeams=gJSF->Env()->GetValue("NNZHBases.Store_Beams",0);
...
//  std::cout << "SetPythiaParameters ... was called." << std::endl;
 ...
 py->SetPMAS(25, 1, 125.0);  // Higgs mass
 py->SetPMAS(25, 2, 0.0043); // Higgs width
...
//  py->SetBRAT(211)=     ;  // h -> u u
  py->SetBRAT(212, 0.00044);  // h -> s s
  py->SetBRAT(213, 0.0268);   // h -> c c
  py->SetBRAT(214, 0.578);    // h -> b b

//  py->SetBRAT(218)=      ;  // h -> e e
  py->SetBRAT(219, 0.000221);  // h -> mu mu
  py->SetBRAT(220, 0.0637);    // h -> tau tau
  py->SetBRAT(222, 0.0856);    // h -> g g
  py->SetBRAT(223, 0.0023);    // h -> gamma gamma
  py->SetBRAT(224, 0.00155);   // h -> gamma Z0
  py->SetBRAT(225, 0.0267);    // h -> Z0 Z0
  py->SetBRAT(226, 0.216);     // h -> W+ W-
