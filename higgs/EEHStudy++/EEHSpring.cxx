...
//  C-tor
// --------------------------
EEHSpring::EEHSpring(const char     *name,
                     const char     *title,
                           EEHBases *bases)
         : JSFSpring(name, title, bases)
{
  fEventBuf = new EEHSpringBuf("EEHSpringBuf",
                               "EEHSpring event buffer",
                               this);
  if (!bases) { 
    SetBases(new EEHBases);
  }
}
...
//  Initialize
// --------------------------
Bool_t EEHSpring::Initialize()
{
  JSFSpring::Initialize();

  if (fFile->IsWritable()) {
    EEHBases *bs = static_cast<EEHBases *>(GetBases());
    ...
    bs->Write();
    last->cd(); 
    cerr << ">>>>>> EEHBases written to file" << endl;
  }
  return kTRUE;
}
...
//  SetPartons
// --------------------------
Bool_t EEHSpringBuf::SetPartons()
{
  TClonesArray &partons = *fPartons;
  EEHBases     *bases   = static_cast<EEHBases *>(
                          static_cast<EEHSpring *>(Module())->GetBases());
  ...
  Double_t mass    = bases->GetMass();            // Higgs mass
  ...
  new (partons[0]) JSFSpringParton(1, ide,   me,    qe, *qp[1], 0, 0,  0,    0,   0,     0);
  new (partons[1]) JSFSpringParton(2,-ide,   me,   -qe, *qp[2], 0, 0,  0,    0,   0,     0);
           ...
           fR_ISR_side(0)
{
  ...
  ins.str("");
  ins.clear();
  ins.str(gJSF->Env()->GetValue("EEHBases.Ecm","500."));       // E_cm (1TeV)
  ins >> fEcmInit;

  ins.str("");
  ins.clear();
  ins.str(gJSF->Env()->GetValue("EEHBases.Beamstrahlung","1")); // BmStr (on)
  ins >> fBeamStr;

  ins.str("");
  ins.clear();
  ins.str(gJSF->Env()->GetValue("EEHBases.BeamWidth","0.002")); // BmStr (on)
  ins >> fBeamWidth;

  ins.str("");
  ins.clear();
  ins.str(gJSF->Env()->GetValue("EEHBases.Bremsstrahlung","1"));// ISR (on)
  ins >> fISR;

  ins.str("");
  ins.clear();
  ins.str(gJSF->Env()->GetValue("EEHBases.Polem","0."));       // electron polarization
  ins >> fPolem;

  ins.str("");
  ins.clear();
  ins.str(gJSF->Env()->GetValue("EEHBases.Polep","0."));       // positron polarization
  ins >> fPolep;
...
//  FullAmplitude()
// --------------------------
Complex_t EEHBases::FullAmplitude()
{
...
//  AmpEEtoEEH()
// --------------------------
Complex_t EEHBases::AmpEEtoEEH(const HELFermion &em,
                               const HELFermion &ep,
                               const HELFermion &e,
                               const HELFermion &eb,
                               const HELScalar  &hs)
...
void EEHBases::Userin()
{
  ...
  if (fBeamStr) {
    stringstream ins(gJSF->Env()->GetValue("EEHBases.BeamstrahlungFilepath",
                                  "/proj/soft/jsf/pro/data/bsdata/"));
    ins >> bsfiledir;

    ins.clear();
    ins.str(gJSF->Env()->GetValue("EEHBases.BeamstrahlungFilename","trc500"));
    ins >> bsfilename;
  ...
  Xh_init( 8,     0.,     4.,        2, "Helin ");
  Xh_init( 9,     0.,     4.,        2, "Helot ");
}
...
//  Userout
// --------------------------
void EEHBases::Userout()
{
  cout << "End of EEHBases----------------------------------- "  << endl
...
//  SelectHelicities
// --------------------------
void EEHBases::SelectHelicities(Double_t &weight)
{
